...
- (UIFont *)customFont{
    if (!_customFont) {
        _customFont = [UIFont systemFontOfSize:self.titleFont];
    }
    return _customFont;
}
...
- (float)scrollLineWidth{
    if (!_scrollLineWidth && _titleArray.count != 0) {
        _scrollLineWidth = self.frame.size.width/_titleArray.count;
    }
    return _scrollLineWidth;
}
...
- (void)addPropertyObserver{
    ...
    [self addObserver:self forKeyPath:@"customFont" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
...
- (void)dealloc{
    ...
    [self removeObserver:self forKeyPath:@"customFont"];
...
- (void)configSubLabel{
    ...
    for (int i = 0;  i < self.titleArray.count; i++) {
        ...
        titleLabel.font = self.customFont;
    ...
    if (_titleArray.count > 0) {
        offset = (ItemWidth - self.scrollLineWidth)/2;
    }
    //滚动条
    _scrollLine = [[UIView alloc]initWithFrame:CGRectMake(offset, ItemHeight - self.scrollLineHeight, self.scrollLineWidth, self.scrollLineHeight)];
...
- (void)selectLabelWithIndex:(NSInteger)index{
    ...
    scrollLineFrame.origin.x = ItemWidth*index + offset;
...
- (void)configSubViewNoChangeIndex{
    ...
    [self selectLabelWithIndex:_curIndex];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    }else if ([keyPath isEqualToString:@"customFont"]){
